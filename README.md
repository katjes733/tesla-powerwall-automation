...
### Exposing the app over the internet (WAN access)

If you want to reach the app from outside your home network (e.g. via a public domain instead of just your LAN), you'll typically put a reverse proxy such as Caddy in front of it to terminate public TLS and forward requests to this app's internal HTTPS listener.

See [docs/reverse-proxy-tls-setup.md](docs/reverse-proxy-tls-setup.md) for a generic walkthrough of that setup (DNS, ACME/DNS-01 certs, router port forwarding) along with the gotchas specific to this app — most notably that its self-signed internal TLS cert requires `tls_insecure_skip_verify` on the proxy hop, and that `ALLOWED_ORIGINS` must include every public hostname you expose (see [Environment variables](#environment-variables) below).
...
>
> If you're exposing the app publicly through a reverse proxy, `ALLOWED_ORIGINS` must also include every public hostname you route to it — see [Exposing the app over the internet (WAN access)](#exposing-the-app-over-the-internet-wan-access).
