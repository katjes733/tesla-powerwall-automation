...
// ignore anything past the first id (verified empirically: --ignore=A,B
// suppresses neither A nor B, while --ignore=A --ignore=B suppresses both).
const args = [
  "audit",
  ...allowlist.map((e) => `--ignore=${e.id}`),
];
