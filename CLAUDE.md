...
**Entries backed by an `audit-allowlist.json` entry are not "fixed forever"** — they're suppressed because the only fix available *right now* is a major bump or doesn't exist yet. Periodically (e.g., whenever `bun run verify`'s audit step is already being touched, or a dependency pass is happening for another reason) re-check whether a non-major-version fix has since shipped upstream for each allowlisted id — a patch/minor release, or the underlying package fixing the flagged transitive dependency. If one has, remove the allowlist entry and the corresponding section below, and let the real fix (`bun update`, a version bump, or an `overrides` pin) land instead of continuing to suppress a now-avoidable finding.
...
**`brace-expansion` — GHSA-mh99-v99m-4gvg / CVE-2026-14257, "DoS via unbounded expansion length causing an out-of-memory process crash"** *(assessed 2026-07-24, likely a false positive — re-check within days, not deferred like the entry above)*

Unlike the `react-router` finding, this one probably isn't a real unresolved vulnerability. The GitHub Advisory Database lists `first_patched_version` as `5.0.8`; `bun.lock` confirms the only 5.x resolution in the tree (pulled in via `minimatch@10.2.5`, used by eslint/nodemon/typescript-eslint) is already `5.0.8` — also npm's current `latest` dist-tag, so there's no newer version to update to. `bun audit`'s own summary line (`<=5.0.7`) matches the advisory's vulnerable range exactly, so it appears to be checking the loosely-declared `^5.0.5` semver range some dependency requests rather than the actual lockfile-pinned resolution, or its local advisory cache simply hadn't caught up yet (the advisory was published only hours before this was assessed). Re-run `bun audit` in a few days and remove this entry (from both here and `audit-allowlist.json`) once it stops flagging an already-patched install.
...
- **Dependency audit** — `bun run verify` ends with `bun run audit` (`scripts/audit.ts`, wrapping `bun audit`) and fails if vulnerabilities are found. When it fails, surface the findings to the user and discuss the appropriate fix together — the options include `bun update` (latest compatible versions), `bun update --latest` (allows major version bumps, may introduce breaking changes), an `overrides` entry in `package.json` to pin a specific transitive dependency, or — for a finding that's genuinely not exploitable here (e.g. a vulnerable code path this app never uses) or has no fix published yet — a new entry in `audit-allowlist.json` with the advisory id, affected package, a written reason, and the assessment date. Do not silently apply overrides, auto-update, or add an allowlist entry without the user's input; also mirror any accepted finding into this file's "Accepted security findings" section for a human-readable record.
